...
export function Shutdown(suspend){
	...
	device.log("Shutting down Govee device... {suspend: " + suspend + "}");

	if(TurnOffOnShutdown || suspend){
		govee.setDeviceState(false);
	} else {
		govee.SetStaticColor(hexToRgb(disabledColor), disabledTemperature);
		govee.SetBrightness(disabledBrightness);
	}
}
...
class GoveeProtocol {
	...
	SetStaticColor(RGBData, colorTemInKelvin = 0) {
		UDPServer.send(JSON.stringify({
			msg: {
				cmd: "colorwc",
				data: {
